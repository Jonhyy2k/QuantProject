...
# Function to search for a specific stock by ticker
def search_stock(client, ticker):
    print(f"[INFO] Searching for stock: {ticker}")
    ...
    # Filter to find the exact ticker or close matches
    matches = []
    exact_match = None
            ...
            for item in response:
                if isinstance(item, dict) and "symbol" in item:
                    if item["symbol"].upper() == ticker.upper():
                        exact_match = item["symbol"]
                        break
                    elif ticker.upper() in item["symbol"].upper():
                        matches.append(item["symbol"])

        if exact_match:
            print(f"[INFO] Found exact match: {exact_match}")
            return exact_match
        elif matches:
            print(f"[INFO] Found {len(matches)} similar matches: {matches[:5]}")
            # Return the first match if no exact match found
            return matches[0]
        else:
            print(f"[ERROR] No matches found for {ticker}")
            return None
    except Exception as e:
        print(f"[ERROR] Error searching for stock: {e}")
        traceback.print_exc()
        return None
...
def save_to_text_file(stock, sigma, rec, details):
    file_path = "Stock_Analysis_Final.txt"
    try:
        ...
        with open(file_path, "a") as file:
            file.write("\n===== SIGMA-BASED TRADING RECOMMENDATION =====\n")
            file.write(f"Generated on: {time.strftime('%Y-%m-%d %H:%M:%S')}\n\n")

            # Get the stock price from the details
            price = details.get("last_price", "N/A")

            # Write the main results
            file.write(f"SYMBOL: {stock}\n")
            file.write(f"PRICE: ${price:.2f}\n")
            file.write(f"SIGMA: {sigma:.5f}\n")
            file.write(f"RECOMMENDATION: {rec}\n\n")

            # Write additional details
            file.write("TECHNICAL DETAILS:\n")
            file.write(f"RSI: {details['rsi_signal']:.1f}\n")
            file.write(f"MACD: {details['macd']:.4f}\n")
            file.write(f"SMA Trend: {details['sma_trend']:.4f}\n")
            file.write(f"Volatility: {details['traditional_volatility']:.4f}\n")

            # Add note if simplified calculation was used
            if details.get("simplified", False):
                file.write("NOTE: Simplified calculation used due to time constraints\n")
...
# Main function to analyze a single stock by ticker with timeout
def analyze_stock_by_ticker(ticker):
    print(f"[INFO] Starting analysis for ticker: {ticker}")

    # Set overall timeout for entire function
    ...
    connection_timeout = 45  # 45 seconds max for connection phase

    while connection_attempts < max_connection_attempts:
        if time.time() - start_time > connection_timeout:
            print("[ERROR] Connection phase timeout reached")
            return False
        ...
        login_timeout = time.time() + 10  # Shorter timeout
        ...
        if connection_attempts < max_connection_attempts:
            print(f"[WARNING] Connection failed, retrying in {connection_attempts * 2} seconds...")
            time.sleep(connection_attempts * 2)  # Shorter backoff
    ...
    try:
        # Check timeout after connection phase
        if time.time() - start_time > max_execution_time * 0.2:  # If 20% of time already used
            print("[WARNING] Connection phase took too long")
            client.disconnect()
            return False

        # Search for the ticker with timeout
        ticker_search_timeout = min(30, max_execution_time * 0.1)  # 10% of total time or 30s
        ticker_search_start = time.time()

        stock_symbol = search_stock(client, ticker)

        # Check if ticker search took too long
        if time.time() - ticker_search_start > ticker_search_timeout:
            print("[WARNING] Ticker search took too long")
            client.disconnect()
            return False

        if not stock_symbol:
            print(f"[ERROR] Could not find stock with ticker: {ticker}")
            client.disconnect()
            return False

        # Check overall timeout
        if time.time() - start_time > max_execution_time * 0.3:  # If 30% of time already used
            print("[WARNING] Initial phases took too long")
            client.disconnect()
            return False

        # Get stock data with timeout
        data_fetch_timeout = min(60, max_execution_time * 0.2)  # 20% of total time or 60s
        data_fetch_start = time.time()

        data = get_stock_data(client, stock_symbol)

        # Check if data fetch took too long
        if time.time() - data_fetch_start > data_fetch_timeout:
            print("[WARNING] Data fetch took too long")
            client.disconnect()
            return False

        if data is None or len(data) < 60:  # Need enough historical data, but reduced requirement
            print(f"[ERROR] Insufficient historical data for {stock_symbol}")
            client.disconnect()
            return False

        # We no longer need the connection for analysis, so disconnect it
        client.disconnect()

        # Check if we still have enough time for analysis
        remaining_time = max_execution_time - (time.time() - start_time)
        if remaining_time < 60:  # Need at least 60s for analysis
            print("[WARNING] Not enough remaining time for full analysis")
            return False

        try:
            # Calculate Sigma score combining all analysis tools
            # This function now has its own internal timeout
            ...
            if analysis:
                ...
                print(f"[INFO] {stock_symbol}: Sigma = {sigma:.5f}, Recommendation = {recommendation}")

                # Check if we used simplified calculation
                if analysis.get("simplified", False):
                    print("[WARNING] Used simplified calculation due to time constraints")

                print(f"[DEBUG] Analysis details: {analysis}")

                # Save the results to a file
                success = save_to_text_file(stock_symbol, sigma, recommendation, analysis)
                if not success:
                    print("[WARNING] Failed to save analysis to file")

                return True
            else:
                print(f"[WARNING] Could not calculate Sigma for {stock_symbol}")
                return False
        except Exception as e:
            print(f"[ERROR] Error analyzing {stock_symbol}: {e}")
            traceback.print_exc()
            return False
    except Exception as e:
...
if __name__ == "__main__":
    try:
        ...
        if len(sys.argv) > 1:
            ticker = sys.argv[1]
            print(f"[INFO] Analyzing ticker from command line: {ticker}")
            analyze_stock_by_ticker(ticker)
        else:
            # Prompt for ticker
            ticker = input("Enter stock ticker to analyze: ")
            analyze_stock_by_ticker(ticker)
    except KeyboardInterrupt:
        print("[INFO] Analysis stopped by user")
    except:
